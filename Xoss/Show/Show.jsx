...
export default class Show extends Component {
  ...
  render() {
    ...
    const w = this.props.width || undefined;
    const h = this.props.height || undefined;
    if (w || h) resize = [];
    if (w) resize.push(w);
    if (h) resize.push(h);
    let options = {
      thumb: this.props.thumb,
      resize: Array.isArray(resize) ? resize.join(',') : resize,
      ...
      this.props.src
        ...
        <div>
          <AutdImage
            ...
            fallback={fallback}
            width={w} height={h}
            src={Xoss.url(this.props.src, options)}
          ...
          <Modal
            ...
            title={I18n('picture display')}
            onCancel={() => this.setState({previewVisible: false})}
          >
            <AutdImage
              preview={false}
              fallback={fallback}
              src={Xoss.url(this.props.src)}
            />
          </Modal>
        </div>
        : <AutdImage width={w} height={h} alt="miss" src={fallback}/>
    );
  }
}
